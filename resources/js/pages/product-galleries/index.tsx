...
function SortableGalleryItem({
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className="group relative flex items-center gap-4 rounded-lg border bg-card p-4 hover:bg-accent/50"
        >
            <button
                {...attributes}
                {...listeners}
            ...
            <img
                ...
                className="h-20 w-32 cursor-pointer rounded object-cover transition-opacity hover:opacity-80"
                onClick={() =>
                    onImageClick(
                        `/storage/${gallery.file_path}`,
                        `${gallery.product.name} - Image ${gallery.order}`,
                    )
                }
            />
            ...
            <div className="flex-1">
                ...
                <div className="mt-1 flex gap-2">
                    <Badge variant="secondary">
                        {gallery.product.supplier.name}
                    </Badge>
                    <Badge variant="outline">
                        {gallery.product.category.name}
                    </Badge>
                </div>
            </div>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    <DropdownMenuItem
                        onClick={() =>
                            router.visit(`/product-galleries/${gallery.id}`)
                        }
                    >
                    ...
                    <DropdownMenuItem
                        onClick={() =>
                            router.visit(
                                `/product-galleries/${gallery.id}/edit`,
                            )
                        }
                    >
                    ...
                    <DropdownMenuItem
                        onClick={() =>
                            onDelete(gallery.id, gallery.product.name)
                        }
...
export default function ProductGalleriesIndex({ galleries }: Props) {
    ...
    const groupedGalleries = galleries.data.reduce(
        (acc, gallery) => {
            ...
            acc[supplierId].categories[categoryId].products[
                productId
    ...
    const handleDragEnd = (productId: number) => (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setSupplierData((prev) => {
                ...
                Object.entries(prev).forEach(([sId, sData]) => {
                    Object.entries(sData.categories).forEach(([cId, cData]) => {
                        if (cData.products[productId]) {
                            targetGalleries =
                                cData.products[productId].galleries;
                ...
                const oldIndex = targetGalleries.findIndex(
                    (item) => item.id === active.id,
                );
                const newIndex = targetGalleries.findIndex(
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 sm:gap-6 sm:p-6">
                ...
                {Object.entries(supplierData).map(
                    ([supplierId, { supplier, categories }]) => (
